// Changed a bit from http://forum.codecall.net/topic/49665-java-random-name-generator/#axzz288wbBJWW
 ...
 */
@Component
public class RandomNameGenerator implements InitializingBean {
	...
	public void refresh() throws IOException {
		...
		input = new StringReader(buildFakeInputFile());
	...
	private boolean VowelLast(String s) {
		return (String.copyValueOf(Vowels).contains(String.valueOf(
	...
	public String compose(int syls) {
		...
		if (syls > 2) {
			if (expectsVowel(pre.get(a))) {
				...
				if (containsVocFirst(mid) == false)
					throw new RuntimeException(
							"Expecting \"middle\" part starting with Vowel, "
			...
			if (expectsVowel(pre.get(a))) {
				...
				if (containsVocFirst(sur) == false)
					throw new RuntimeException(
							"Expecting \"suffix\" part starting with Vowel, "
									+ "but there is none. You should add one, or remove requirement for one.. ");
			}
			if (expectsConsonant(pre.get(a))) {
				expecting = 2;
		...
		for (int i = 0; i < b.length - 2; i++) {
			...
			do {
				...
				// +expecting+" VowelF:"+VowelFirst(mid.get(b[i]))+" syl: "+mid.get(b[i]));
					...
					&& VowelFirst(pureSyl(mid.get(b[i]))) == false
					|| expecting == 2
			...
			if (VowelLast(pureSyl(mid.get(b[i]))) && allowVocs(mid) == false
					&& syls > 3)
				throw new RuntimeException(
		...
		do {
			c = (int) (Math.random() * sur.size());
				...
				&& hatesPreviousVowels(sur.get(c)) || last == 2
